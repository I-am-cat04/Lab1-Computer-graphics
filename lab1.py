...
def x_loop_line_fixed(image, x0, y0, x1, y1, color):
...
def bresenhams_algorithm(image, x0, y0, x1, y1, color):
...
#     bresenhams_algorithm(img_mat, x0, y0, x1, y1, 255)
#
# img = Image.fromarray(img_mat, mode='RGB')
# img.save('img_bresenhams_algorithm.png')

# Счёт информации о вершинах и полигонах модели
...
for k in range(len(f)):
    ...
    bresenhams_algorithm(img_mat, x0, y0, x1, y1, 255)
    bresenhams_algorithm(img_mat, x1, y1, x2, y2, 255)
    bresenhams_algorithm(img_mat, x2, y2, x0, y0, 255)




img = ImageOps.flip(Image.fromarray(img_mat, mode='RGB'))
img.save('img_task6.png')